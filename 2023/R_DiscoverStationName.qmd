...
Raw <- arrow::read_parquet("~/Dropbox/DataClean/RetailJPN/AllPrefectture.parquet") |> 
  ...
  mutate(
    AveQuartely = mean(Quartely),
    AveTenure = mean(Tenure),
    AveDistanceStation = mean(DistanceStation),
    AveSize = mean(Size),
    AveYouseki = mean(Youseki),
    AveKenpei = mean(Kenpei),
    AveRoomNum = mean(RoomNum),
    AveZoneHouse = mean(ZoneHouse),
    AveZoneBusiness = mean(ZoneBusiness),
    AveZoneFactory = mean(ZoneFactory),
    AveRoomL = mean(RoomL),
    AveRoomD = mean(RoomD),
    AveStructureSRC = mean(StructureSRC),
    AveReform = mean(Reform),
    .by = StationName
  ) |> 
  select(Quartely,Price:District,AveQuartely:AveReform) |> 
  na.omit()
...
Est <- function(q){
  ...
  TempLinearAIPW <- (1-q)*Q_LinearAIPW + 
    if_else(
      PredLinearAIPW$response[Group == 3] >= Q_LinearAIPW,
      ScoreAIPW[Group == 3]- Q_LinearAIPW,
      0
